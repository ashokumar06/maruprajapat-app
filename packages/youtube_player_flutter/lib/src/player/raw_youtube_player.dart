        ...
        onWebViewCreated: (webController) {
          ...
          webController
            ...
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                debugPrint('StateChange => $args');
                final state = int.tryParse(args.first.toString()) ?? -1;
                switch (state) {
                  case -1:
                    controller!.updateValue(
            ...
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                debugPrint('PlaybackQualityChange => $args');
                controller!.updateValue(
                  controller!.value
                      .copyWith(playbackQuality: args.first.toString()),
                );
              },
            )
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                debugPrint('PlaybackRateChange => $args');
                controller!.updateValue(
                  controller!.value.copyWith(
                    playbackRate: double.tryParse(args.first.toString()) ?? 1.0,
                  ),
                );
              },
            )
            ..addJavaScriptHandler(
              handlerName: 'Errors',
              callback: (args) {
                debugPrint('Errors => $args');
                controller!.updateValue(
                  controller!.value.copyWith(
                    errorCode: int.tryParse(args.first.toString()) ?? 0,
                  ),
                );
              },
            )
            ...
            ..addJavaScriptHandler(
              ...
              callback: (args) {
                debugPrint('VideoTime => $args');
                final position = double.tryParse(args.first.toString()) ?? 0;
                final buffered = double.tryParse(args.last.toString()) ?? 0;
                controller!.updateValue(
                  controller!.value.copyWith(
    ...
    <body>
        ...
        <script>
            ...
            function onYouTubeIframeAPIReady() {
                player = new YT.Player('player', {
                    ...
                    events: {
                        ...
                        onPlaybackQualityChange: function(event) { window.flutter_inappwebview.callHandler('PlaybackQualityChange', String(event.data)); },
                        onPlaybackRateChange: function(event) { window.flutter_inappwebview.callHandler('PlaybackRateChange', String(event.data)); },
                        onError: function(error) { window.flutter_inappwebview.callHandler('Errors', String(error.data)); }
                    },
                });
            }
            ...
            function sendPlayerStateChange(playerState) {
                ...
                window.flutter_inappwebview.callHandler('StateChange', String(playerState));
                if (playerState == 1) {
            ...
            function sendVideoData(player) {
                var videoData = {
                    'duration': String(player.getDuration()),
                    'title': String(player.getVideoData().title),
                    'author': String(player.getVideoData().author),
                    'videoId': String(player.getVideoData().video_id)
                };
                window.flutter_inappwebview.callHandler('VideoData', JSON.stringify(videoData));
            }

            function startSendCurrentTimeInterval() {
                timerId = setInterval(function () {
                    window.flutter_inappwebview.callHandler('VideoTime', String(player.getCurrentTime()), String(player.getVideoLoadedFraction()));
                }, 100);
            }
